...
<div x-cloak x-show="openDropdown" @click.away="openDropdown = false"
    ...
    <a href="{{ route('home.index') }}"
        class="block w-full py-2 font-bold transition px-7 hover:text-white hover:bg-2025-2 whitespace-nowrap">Halaman
        Utama</a>
    <form action="{{ route('logout') }}" method="post">
        @csrf
        <button type="submit"
            class="block w-full py-2 font-bold transition px-7 hover:text-white hover:bg-2025-2">Logout</button>
    </form>
</div>
