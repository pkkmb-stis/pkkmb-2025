<div>
    <div x-data="{ openedit: @entangle('openedit').live, showModalDelete: false, materiTitle: '', materiId: '', showModalDetail: @entangle('showModalDetail').live }">
        <x-card class="mb-8">
            <div class="flex items-center justify-between mb-3">
                <h5 class="text-xl font-normal text-gray-700 font-bohemianSoul">Daftar Materi PKKMB</h5>
                @can(PERMISSION_ADD_MATERI)
                    @livewire('admin.informasi.materi.add')
                @endcan
            </div>
            <div class="mb-3">
            ...
            <x-input wire:model.live.debounce.200ms="search" type="text"
                placeholder="Cari berdasarkan judul materi ..." class="block w-full mb-3 placeholder-gray-400" />
                    ...
                    <slot>
                                ...
                                <td class="px-6 py-3 text-center">
                                    ...
                                    @can(PERMISSION_DELETE_MATERI)
                                        <x-button class="rounded-3xl bg-merah-700 hover:bg-merah-hover mx-0.5"
                                            x-on:click="showModalDelete = true; materiTitle = '{{ $row->title }}'; materiId = '{{ $row->id }}'">
                                            Delete
                                        </x-button>
                                    @endcan
                                </td>
                        ...
                        @empty
                            <tr class="border-b border-gray-200 hover:bg-blueGray-100">
                                <td colspan="4" class="px-6 py-3 italic text-center text-md">Tidak ada materi</td>
                            </tr>
                        @endforelse
            ...
            <div class="mt-3">
                {{ $materi->onEachSide(ON_EACH_SIDE)->links(DEFAULT_PAGINATION) }}
            </div>
        </x-card>
        ...
        @can(PERMISSION_DELETE_MATERI)
            <div x-cloak x-show="showModalDelete">
                <x-modal>
                    <x-modal.warning>
                        <x-slot name="title">
                            <h5 class="font-bold">Hapus materi</h5>
                        </x-slot>

                        <div>
                            Apakah kamu yakin untuk menghapus <b x-text="materiTitle"></b>?
                        </div>
                        ...
                        <x-slot name="footer">
                            ...
                            <x-button class="rounded-3xl bg-merah-700 hover:bg-merah-hover" :tagA="false"
                                x-on:click="showModalDelete = false; $wire.hapus(materiId)">Ya, yakin</x-button>
                        </x-slot>
                    </x-modal.warning>
                </x-modal>
            </div>
        ...
        @can(PERMISSION_UPDATE_MATERI)
            @include('admin.informasi.materi.edit')
        @endcan
    </div>
</div>
