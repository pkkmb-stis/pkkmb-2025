<div>
    <x-card x-data="{ showModalDelete: false, 'galleryTitle': '', 'galleryId': '' }">
        ...
        <div class="grid mb-3 lg:grid-cols-2 lg:gap-6 gap-y-3">
            ...
            <x-select-form wire:model.blur="inHome">
        ...
        <x-input wire:model.live.debounce.200ms="title" type="text" placeholder="{{ $placeholder }}"
            ...
            <slot>
                @forelse ($gallery as $g)
                    <tr class="border-b border-gray-200 hover:bg-blueGray-100 {{ $loop->even ? 'bg-gray-50' : '' }}"
                        x-data="{}"
                        x-on:click="if (window.innerWidth <= 640) { $wire.dispatch('openDetailGallery', { galleryId: {{ $g->id }} })
                        }">
                        ...
                        <td class="hidden px-6 py-3 text-center sm:table-cell">
                            <x-button wire:click="$dispatch('openDetailGallery', {{ $g->id }})"
