...
public function addAnggota()
{
    if (!$this->canAddDeleteAnggota) {
        $this->dispatch('updated', 
            title: 'Kamu tidak memiliki akses untuk menambah anggota kelompok', 
            icon: 'error', 
            iconColor: 'red'
        );
        return;
    ...
    try {
        $user = User::doesntHave('kelompok')
            ->find($this->selectedUser['id']);
            
        if (!$user) {
            $this->dispatch('updated', 
                title: 'User sudah masuk dikelompok lain', 
                icon: 'error', 
                iconColor: 'red'
            );
            return;
        ...
        $user->update(['kelompok_id' => $this->kelompok->id]);
...
public function removeAnggota($id)
{
    if (!$this->canAddDeleteAnggota) {
        $this->dispatch('updated', 
            title: 'Kamu tidak memiliki akses untuk menghapus anggota kelompok', 
            icon: 'error', 
            iconColor: 'red'
        );
        return;
...
public function updateKelompok()
{
    ...
    if (!$this->canUpdateKelompok) {
        $this->dispatch('updated', 
            title: 'Kamu tidak memiliki akses untuk mengupdate informasi kelompok', 
            icon: 'error', 
            iconColor: 'red'
        );
        return;
    ...
    try {
        ...
        $this->dispatch('updated', 
            title: 'Berhasil mengupdate kelompok', 
            icon: 'success', 
            iconColor: 'green'
        );
        ...
        $this->dispatch('updated', 
            title: 'Gagal mengupdate kelompok', 
            icon: 'error', 
            iconColor: 'red'
        );
    }
