...
class Show extends Component
{
    ...
    public function update()
    {
        ...
        if (!userHasPermission(PERMISSION_UPDATE_EVENT)) {
            $this->dispatch('updated', 
                title: 'Kamu tidak memiliki akses untuk update event absensi', 
                icon: 'error', 
                iconColor: 'red'
            );
            return;
        ...
        try {
            $event = Event::find($this->selected);
    ...
    public function hapus($id)
    {
        if (!userHasPermission(PERMISSION_DELETE_EVENT)) {
            $this->dispatch('updated', 
                title: 'Kamu tidak memiliki akses untuk delete event absensi', 
                icon: 'error', 
                iconColor: 'red'
            );
            return;
