...
public function revokePermission()
{
    if (!userHasPermission(PERMISSION_UPDATE_AKSES_ADMIN)) {
        $this->dispatch('updated', 
            title: 'Kamu tidak memiliki akses untuk menghapus permission dari user ini', 
            icon: 'error', 
            iconColor: 'red'
        );
        return;
    ...
    try {
        ...
        $this->dispatch('updated', 
            title: "Permission {$this->permissionToRevoke} berhasil dihapus", 
            icon: 'success', 
            iconColor: 'green'
        );
...
public function addPermission()
{
    if (!userHasPermission(PERMISSION_UPDATE_AKSES_ADMIN)) {
        $this->dispatch('updated', 
            title: 'Kamu tidak memiliki akses untuk menambah permission dari user ini', 
            icon: 'error', 
            iconColor: 'red'
        );
        return;
    ...
    try {
        ...
        $this->dispatch('updated', 
            title: "Permission {$this->permissionToAdd} berhasil ditambahkan", 
            icon: 'success', 
            iconColor: 'green'
        );
