...
public function revokeRole()
{
    if (!userHasPermission(PERMISSION_UPDATE_AKSES_ADMIN)) {
        $this->dispatch('updated', 
            title: 'Kamu tidak memiliki akses untuk menghapus role dari user ini', 
            icon: 'error', 
            iconColor: 'red'
        );
        return;
    ...
    try {
        ...
        $this->dispatch('updated', 
            title: "Role {$this->roleToRevoke} berhasil dihapus", 
            icon: 'success', 
            iconColor: 'green'
        );
    ...
    public function addRole()
    {
        if (!userHasPermission(PERMISSION_UPDATE_AKSES_ADMIN)) {
            $this->dispatch('updated', 
                title: 'Kamu tidak memiliki akses untuk menambah role dari user ini', 
                icon: 'error', 
                iconColor: 'red'
            );
            return;
        ...
        try {
            ...
            $this->dispatch('updated', 
                title: "Role {$this->roleToAdd} berhasil ditambahkan", 
                icon: 'success', 
                iconColor: 'green'
            );
