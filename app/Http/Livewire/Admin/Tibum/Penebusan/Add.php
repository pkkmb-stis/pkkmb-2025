...
class Add extends Component
{
    ...
    public function submit()
    {
        ...
        if (!userHasPermission(PERMISSION_ADD_PENEBUSAN))
            $this->dispatchBrowserEvent('updated', ['title' => 'Kamu tidak memiliki akses untuk menambah penebusan', 'icon' => 'error', 'iconColor' => 'red']);        //* Validasi
        else {
            ...
            try {
                ...
                $this->emit('reloadTablePenebusanAdmin');
            } catch (\Throwable $th) {
                dd($th->getMessage());
                $this->dispatchBrowserEvent('updated', ['title' => 'Penebusan gagal ditambahkan', 'icon' => 'error', 'iconColor' => 'red']);
            }
        }
    }
