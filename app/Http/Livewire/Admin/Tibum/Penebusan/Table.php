...
namespace App\Http\Livewire\Admin\Tibum\Penebusan;
...
class Table extends Component
{
    ...
    protected $listeners = ['reloadTablePenebusanAdmin' => '$refresh'];
    ...
    public function update()
    {
        ...
        if (!userHasPermission(PERMISSION_UPDATE_PENEBUSAN))
            $this->dispatchBrowserEvent('updated', ['title' => 'Kamu tidak memiliki akses untuk edit penebusan', 'icon' => 'error', 'iconColor' => 'red']);
        else {
            ...
            try {
                ...
                $this->resetAll();
                $this->dispatchBrowserEvent('updated', ['title' => 'Berhasil mengubah status penebusan', 'icon' => 'success', 'iconColor' => 'green']);
            } catch (\Throwable $th) {
                dd($th->getMessage());
                $this->dispatchBrowserEvent('updated', ['title' => "Gagal mengubah status penebusan", 'icon' => 'error', 'iconColor' => 'red']);
            }
        }
        $this->resetAll();
    }
    ...
    public function destroy(Penebusan $penebusan)
    {
        if (!userHasPermission(PERMISSION_DELETE_PENEBUSAN))
            $this->dispatchBrowserEvent('updated', ['title' => 'Kamu tidak memiliki akses untuk menghapus penebusan', 'icon' => 'error', 'iconColor' => 'red']);
        else {
            try {
                ...
                $this->dispatchBrowserEvent('updated', ['title' => 'Berhasil menghapus penebusan', 'icon' => 'success', 'iconColor' => 'green']);
            } catch (\Throwable $th) {
                $this->dispatchBrowserEvent('updated', ['title' => 'Gagal menghapus penebusan', 'icon' => 'error', 'iconColor' => 'red']);
            }
        }
        $this->resetAll();
    }
