...
class Add extends Component
{
    ...
    public function submit()
    {
        if (!userHasPermission(PERMISSION_ADD_GALLERY))
            $this->dispatchBrowserEvent('updated', ['title' => 'Kamu tidak memiliki akses untuk menambah gallery', 'icon' => 'error', 'iconColor' => 'red']);
        else {
            $this->validate([
                'title' => 'required',
                'category' => 'required',
            ...
            if ($this->category == CATEGORY_GALLERY_VIDEO) {
                ...
                // jika foto maka pindahkan filenya
            ...
            try {
                ...
                $this->dispatchBrowserEvent('updated', ['title' => 'Berhasil menambahkan ' . getCategoryGallery($this->category), 'icon' => 'success', 'iconColor' => 'green']);
                $this->emit('refreshAdminGallery', $this->category);
            } catch (\Throwable $th) {
                $this->dispatchBrowserEvent('updated', ['title' => "Gagal menambah " . getCategoryGallery($this->category), 'icon' => 'error', 'iconColor' => 'red']);
            }
            $this->resetAll();
        }
    }
