    ...
    public function ubahPassword()
    {
        ...
        if (Hash::check(md5($this->password_lama), auth()->user()->password)) {
            try {
                ...
                $this->dispatchBrowserEvent('updated', ['title' => "Password berhasil diubah", 'icon' => 'success', 'iconColor' => 'green']);
            } catch (\Throwable $th) {
                $this->dispatchBrowserEvent('updated', ['title' => "Gagal mengubah password", 'icon' => 'success', 'iconColor' => 'green']);
            }
            $this->resetAll();
        } else
