...
class Upload extends Component
{
    ...
    protected $listeners = ['mabaUploadPenebusan'];
    ...
    public function mabaUploadPenebusan(Penebusan $penebusan)
    {
        ...
        if ($this->selected->user_id != auth()->user()->id) {
            $this->dispatchBrowserEvent('updated', ['title' => 'Tidak dapat upload penebusan', 'icon' => 'error', 'iconColor' => 'red']);
            return;
        }
        $this->openedit = true;
    }
    ...
    public function update()
    {
        ...
        if (now()->gt($this->selected->deadline)) {
            ...
            $this->dispatchBrowserEvent('updated', ['title' => 'Kamu sudah terlambat. Hubungi Tibum', 'icon' => 'error', 'iconColor' => 'red']);
        } else {
            // siapkan nama file
            $user = auth()->user();
            ...
            try {
                ...
                $this->resetAll();
                $this->dispatchBrowserEvent('updated', ['title' => 'Berhasil mengupload tugas penebusan', 'icon' => 'success', 'iconColor' => 'green']);
            } catch (\Throwable $th) {
                $this->dispatchBrowserEvent('updated', ['title' => 'Gagal mengupload tugas penebusan', 'icon' => 'error', 'iconColor' => 'red']);
            }
        }
